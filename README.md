# fast-notepad<br>
A revival of Igor's Fast Notepad 2.0.9 for Android (aka Taxaly Noteme)<br>
Now supporting Android 5.0 to 16<br>
<br>
NOTE: To uninstall any version of Fast Notepad without losing your notes:<br>
First go into Settings, Storage location, and make sure Internal memory is selected.<br>
Your notes will be migrated to that location and preserved on uninstall.<br>
<br>
Please open an issue if you have any problems with file/folder names, <br>
language errors in any language, or anything else!<br>
<br>
<br>
<b>v2.1.6 -- 2026-05-29</b><br>
Fixed the Storage Location dialog so the manual path is updated immediately<br>
Downgrade to v2.1.0 is allowed without uninstalling.<br>
<br>
<b>v2.1.5 -- 2026-04-30</b><br>
Fixed error handling when saving files<br>
Enhanced filename sanitization<br>
Sanitize folder names<br>
If a file is deleted from storage while it's being edited, re-saving works now.<br>
Will now overwrite modified files instead of renaming them, unless the name has changed.<br>
Changed folder name input to single line to avoid embedded carriage returns.<br>
<br>
<b>v2.1.4 -- 2026-03-18</b><br>
Internal memory is now the default storage location on a fresh install.<br>
This prevents notes from getting lost on uninstall.<br>
Tweaked encrypted note colors<br>
Restored landscape layouts to the source tree<br>
Recreated all the git tags and releases based on (I think) the correct commits<br>
<br>
<b>v2.1.3 -- 2026-01-11</b><br>
Fixed search colors<br>
Cleaned up old layouts<br>
<br>
<b>v2.1.2 -- 2026-01-10</b><br>
Fixed encrypted note color scheme<br>
Minimum API 21 (Android 5.0)<br>
<br>
<b>v2.1.1 -- 2025-12-11</b><br>
Update to API 35 (Android 15)<br>
Minimum API 19 (Android 4.4)<br>
<br>
<b>v2.1.0 -- 2025-12-06</b><br>
Update to API 29 (Android 10)<br>
Minimum API 19 (Android 4.4)<br>
<br>
<b>v2.0.9</b><br>
Pre-fork, API 23 (Android 6)<br>
Minimum API 8 (Android 2.2)<br>
